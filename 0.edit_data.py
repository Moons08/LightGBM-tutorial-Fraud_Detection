...
def edit_data(base, train=True):
    for idx, df in enumerate(base):
        ...
        unique = df.groupby(
            'ip')[['app', 'device', 'os', 'channel', 'hour']].nunique().reset_index()
        unique.columns = list(
            map(lambda x: 'unique_{}'.format(x), unique.columns[:]))
        unique.rename(columns={'unique_ip': 'ip'}, inplace=True)
        df = pd.merge(df, unique, on='ip')
        del unique

        # 동일 ip-device-os 그룹 / 사용한 app과 channel의 갯수
        IDO = df.groupby(by=['ip', 'device', 'os'])[
            ['app', 'channel']].nunique()
        ...
        if train:
            ...
            if not os.path.isfile('./data/edited.csv'):
                ...
                df.to_csv('./data/edited.csv', mode='a',
                          header=False, index=False)

            if idx % 10 == 0:
                print(idx, "th train done!")

        else:

            if not os.path.isfile('./data/edited_test.csv'):
                df.to_csv('./data/edited_test.csv',
                          header=df.columns, index=False)
            else:
                df.to_csv('./data/edited_test.csv', mode='a',
                          header=False, index=False)

            if idx % 10 == 0:
                print(idx, "th test done!")


with timer_memory('edit train'):
    print('edit start')
    base = pd.read_csv('train.csv', chunksize=3000000)
    edit_data(base)

with timer_memory('edit test'):
    base = pd.read_csv('test.csv', chunksize=3000000)
